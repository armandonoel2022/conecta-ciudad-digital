-- Corregir función para usar monto fijo residencial de 325 pesos sin variación aleatoria
CREATE OR REPLACE FUNCTION public.auto_generate_monthly_bills()
RETURNS integer
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  due_date DATE;
  base_amount INTEGER := 32500; -- $325 pesos en centavos (residencial por defecto)
BEGIN
  ...
  LOOP
    INSERT INTO garbage_bills (
      ...
      current_month_start,
      current_month_end,
      base_amount, -- Monto fijo de 325 pesos sin variación
      due_date,
      'pending'
    );
